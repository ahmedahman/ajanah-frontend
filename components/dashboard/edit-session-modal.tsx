...
export function EditSessionModal({
  ...
  useEffect(() => {
    if (session) {
      ...
      setSessionType(session.session_type ?? "TALK");
      setStartTime(
        session.start_time
      ...
      setLocation(session.location_label ?? "");
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await agendaAPI.updateSession(session.id, {
        title,
        description,
      ...
      if (res.success) {
        ...
        setError(res.error?.message ?? "Failed to update session.");
      }
    } catch {
